...
- **Jukebox handoff (2026-08-16, in-app overlay — no longer a redirect):** 5s after the grading-break slide becomes current, `JukeboxBreakOverlay` mounts as a `z-[70]` overlay on top of it (same origin, no iframe — Spotify still refuses iframe embedding, which is exactly why this is a same-app overlay rather than an embed). Return is via the ported Jukebox's `b`-hold, which now calls `onExitToShow()` instead of navigating anywhere; `Display.jsx`'s `advanceAfterBreak()` calls the `advance_show` Supabase RPC, which auto-detects whether this is the show's final grading break the same way as before — if the show's last slide is type `winner-reveal` AND no `grading-break` slides remain after the current position, it jumps straight to the last slide; otherwise advances by 1. (Still no per-slide flag — structural detection off the slide list.) See `references/features.md`'s Trivia Jukebox Integration section for the full mechanism.
...
**Data shape** (per overlay, in `slide.data.overlays[]`): `{ id, kind: 'text'|'image', x, y, w, rotation, z, ...kind-specific }`. Text carries `text, fontFamily ('display'|'body'|literal), fontSize, color (hex, a per-theme token 'text'/'textMuted'/'accent'/'highlight'/'shinyAccent', or the fixed 'gold' token → `client/src/lib/shinyGold.js`, theme-independent), align, weight, italic, shadow`. (In the picker the `shinyAccent` swatch is labelled "Theme Pop" and `gold` "Gold — the shiny signal"; `gold` added 2026-07-07. Resolution is identical in `OverlayLayer.jsx` and `SlideCanvasEditor.jsx` — WYSIWYG.) (`italic` and `shadow` were added 2026-07-06 with the design toolbar; both are optional and degrade gracefully when absent on older overlays.) Image carries `mediaUrl`. `fontFamily`/`color` tokens resolve through the active theme; anything else passes through literally. Overlay text does NOT go through `autoFitText` — the host chose the size.
