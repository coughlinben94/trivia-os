...
`ParticleBackground` takes `{ theme, slideIndex }`. For the 12 gradient themes it renders `<BreathingGradient>`; for a ring theme it renders `<RingAmbient>` with that theme's worldData plus `slideIndex`; for the 8 bespoke themes it looks up `AMBIENT_MAP[theme.id]` (see the rework section at top). Components render under one `absolute inset-0` wrapper.
