...
export function useShow() {
  ...
  // Live team count for baking team-picker parts — the one live lookup the
  // shared stepping module needs, injected so that module stays Supabase-free.
  async function fetchTeamCount() {
    const { data, error } = await supabase.from('teams').select('id').eq('show_id', show.id)
    if (error) console.error('[useShow] team count fetch failed:', error)
    return data?.length ?? 0
  }

  function bakeTeamPickerParts(slides, slide) {
    return bakeParts(slides, slide, fetchTeamCount)
  }
  ...
  async function applyStepPatch(patch) {
    setShow(s => ({
      ...
      showState: {
        ...s.showState,
        ...(patch.current_slide_id !== undefined ? { currentSlideId: patch.current_slide_id } : {}),
        ...(patch.current_slide_index !== undefined ? { currentSlideIndex: patch.current_slide_index } : {}),
        ...(patch.answer_reveal !== undefined ? { answerReveal: patch.answer_reveal } : {}),
      },
    }))
    await updateShowRow(show.id, patch)
  }
  ...
  // Thin wrappers — the actual step DECISION logic lives in
  // client/src/lib/slideStepping.js so /display can run the identical
  // machinery against its own raw show shape (see that file's header).
  async function nextSlide() {
    if (!show) return
    markLocalNav()
    const patch = await computeNextStep(
      { slides: show.slides, currentSlideIndex: show.showState.currentSlideIndex, currentSlideId: show.showState.currentSlideId },
      fetchTeamCount
    )
    if (!patch) return
  ...
  async function prevSlide() {
    if (!show) return
    markLocalNav()
    const patch = await computePrevStep(
