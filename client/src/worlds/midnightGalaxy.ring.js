...
export const midnightGalaxyRing = {
  ...
  stations: [
    { key: 'ringed planet', prim: 'ring', hue: 256, accent: false, maxDetail: 2 }, // was st9 — radial mass; maxDetail 2026-08-26 (ring-verify Bug A: measured 7, over the 2-5 band — see RingAmbient.jsx's maxDetail comment)
    { key: 'spiral galaxy', prim: 'lens', hue: 170, accent: false, companionKind: 'dots', maxDetail: 1 }, // was st3 — spiral/disc; hue 276->170 2026-08-12, synced from world-07-ring.html (Ben: "still hate the oval purple color"); companionKind 2026-08-13 synced (Ben: "cluster of random blue shapes... trashed and redone"); maxDetail 2026-08-26 (Bug A: measured 8)
    ...
    { key: 'amber planet', prim: 'ring', variant: 'dust', hue: 28, accent: true, maxDetail: 1 }, // was st0, was 'orange nebula'/blob+ring:true — rebuilt 2026-08-13 as a dust-ringed planet on st0's `ring` anatomy (Ben: "that saturn like planet needs a reworking"); synced from world-07-ring.html, see that file's st3 comment for the family-spacing tradeoff flag; maxDetail 2026-08-26 (Bug A: measured 8)
    { key: 'lit planet', prim: 'planet', hue: 140, accent: false, region: 'aurora', maxDetail: 1 }, // NEW — radial mass; `region` replaces greenWash 2026-08-16, synced from world-07-ring.html (see ringPrimitives.js's SKY_REGIONS block for why the wash was retired outright rather than re-tuned a fifth time); maxDetail 2026-08-26 (Bug A: measured 7)
    { key: 'pulsar', prim: 'pulsar', hue: 120, accent: false, region: 'aurora', regionSource: true, noCompanion: true }, // NEW — radiant burst; regionSource: the pulsar IS the light source the aurora sky leans toward (its own glow gets the anchored light-field); noCompanion 2026-08-13 (Ben: "still a background circle" — this station's own companion, mis-marked as bleed by round-6, see world-07-ring.html's identical comment)
    ...
    { key: 'binary pair', prim: 'binary', hue: 214, accent: false }, // was st7 — radial mass; orangeWash REMOVED 2026-08-16 (synced from world-07-ring.html): an orange wash over a blue object had no visible cause, which is the whole reason it read as a filter. The warm region moved to st10, whose own object (the supernova) is already amber — fixed at the data level, not by re-tuning alpha.
    { key: 'asteroid field', prim: 'asteroidField', hue: 160, accent: false }, // NEW — scattered cluster; fillCorner (Ben: "need something here" on the bottom-left) REMOVED 2026-08-26 (ring-verify Bug B + Bug A): it drew via makeOccluder, a subtractive element — banned outright on this station by spec §7.2 (st9 is one of the bottom-third-by-arc quietest stations), and cutting it also brings st9's own element count from 6 into the 2-5 band (Bug A) in the same edit. See RingAmbient.jsx's fillCorner removal for the full reasoning.
    { key: 'record', prim: 'record', hue: 300, accent: false, region: 'disco', regionSource: true, maxDetail: 1 }, // was st12 — swapped with the supernova 2026-08-16 (Ben: "it can just flip it with another station"), synced from world-07-ring.html; see that file's entry for the full spacing arithmetic. At st12 the disc sat 1 station from the ringed planet (st0). Here both neighbours are the world's two least-round silhouettes (asteroid field, aurora ribbon). Residual, flagged not hidden: d(10,8)=2 to the binary pair — >=3 from every radial mass is arithmetically impossible at PANES=13 with {0,3,4,8} fixed. Also flagged: st10's arc is quieter than st12's (11.8 vs 21.3), so the record's own headline renders dimmer; the disco region + source glow are weight-driven (full at the member station) and lose nothing. regionSource: the record's label lights the disco sky, same contract as the pulsar/supernova. accent:false — warm-complementary cap (<=3) already met by st3/st6/st12. maxDetail 2026-08-26 (Bug A: measured 7, plus the st9 spanning-headline bleed-in that Bug C's placement fix also removes)
    { key: 'aurora ribbon', prim: 'ribbon', hue: 196, accent: false, bandUpper: false, maxDetail: 1 }, // was st11 (unchanged) — elongated streak; bandUpper:false synced from world-07-ring.html (Ben: "move to bottom right"); maxDetail 2026-08-26 (Bug A: measured 6)
    { key: 'supernova', prim: 'spikes', hue: 36, accent: true, region: 'ember', regionSource: true, maxDetail: 1 }, // was st2, then st10; swapped with the record 2026-08-16 — radiant burst, family {5,12} now d=6 (was {5,10}=5). The loudest accent object lands on a louder arc slot (21.3 vs st10's 11.8) — suits it. `region:'ember'` (2026-08-16, moved from st8): an exploding star is an undeniable source for a warm sky; single-station core, so st11 previews at 0.25 and st0 fades out at 0.5 (skyRegionWeights). maxDetail 2026-08-26 (Bug A: measured 7)
  ],
}
