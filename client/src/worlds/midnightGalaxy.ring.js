...
export const midnightGalaxyRing = {
  ...
  stations: [
    ...
    { key: 'lit planet', prim: 'planet', hue: 140, accent: false, region: 'aurora' }, // NEW — radial mass; `region` replaces greenWash 2026-08-16, synced from world-07-ring.html (see ringPrimitives.js's SKY_REGIONS block for why the wash was retired outright rather than re-tuned a fifth time)
    ...
    { key: 'supernova', prim: 'spikes', hue: 36, accent: true, region: 'ember', regionSource: true }, // was st2 — radiant burst; noWash flag removed 2026-08-13, synced from world-07-ring.html — the far-layer wash loop it opted out of is gone entirely now. `region:'ember'` 2026-08-16: the warm region moved here from st8 because an exploding star is an undeniable source for a warm sky; single-station core, so st9 previews at 0.25 and st11 fades out at 0.5 (skyRegionWeights).
    ...
    { key: 'record', prim: 'record', hue: 300, accent: false, region: 'disco', regionSource: true }, // NEW 2026-08-16 — station 13, the jukebox grading-break's own slot (Ben: "it needs to have its own ring slot"). Synced from world-07-ring.html; see that file's entry for the silhouette-spacing tradeoff flagged for Ben. regionSource: the record's own label is the light source for the disco sky, same contract as the pulsar/supernova. accent:false — the world's warm-complementary accent cap (<=3) is already met by st3/st6/st10.
  ],
}
