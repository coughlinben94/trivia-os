...
export const midnightGalaxyRing = {
  ...
  stations: [
    ...
    { key: 'record', prim: 'record', hue: 300, accent: false, region: 'disco', regionSource: true }, // was st12 — swapped with the supernova 2026-08-16 (Ben: "it can just flip it with another station"), synced from world-07-ring.html; see that file's entry for the full spacing arithmetic. At st12 the disc sat 1 station from the ringed planet (st0). Here both neighbours are the world's two least-round silhouettes (asteroid field, aurora ribbon). Residual, flagged not hidden: d(10,8)=2 to the binary pair — >=3 from every radial mass is arithmetically impossible at PANES=13 with {0,3,4,8} fixed. Also flagged: st10's arc is quieter than st12's (11.8 vs 21.3), so the record's own headline renders dimmer; the disco region + source glow are weight-driven (full at the member station) and lose nothing. regionSource: the record's label lights the disco sky, same contract as the pulsar/supernova. accent:false — warm-complementary cap (<=3) already met by st3/st6/st12.
