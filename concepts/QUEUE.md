...
- **grilled**: Ben + Claude have run a daytime grill session and locked a one-paragraph brief with explicit beginning/middle/end story beats (per `references/round-journeys.md`'s "It's a story, not just a mood transition"). Cross-theme handoff briefs name both themes explicitly. **Before locking, cross-check the brief against premises already locked elsewhere** — grep prior prototypes' notes blocks and earlier QUEUE entries for settled facts the new brief must not contradict, especially camera POV, framing, and who the audience is supposed to be. A locked brief is not the same as a consistent one: on 2026-07-22 a full grill session produced the most detailed brief in this file (`d2757ad`), and it specified a third-person wide shot with a visible hull against a premise already written down in `meteor-shower-liftoff-pov-teampicker.html` — that the audience rides *inside* the ship. Nobody compared the two. Three iterations burned before the contradiction surfaced as "remember, we are in the spaceship."
