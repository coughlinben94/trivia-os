...
4. **Propose new elements — never build them unasked.** Ideas are welcome: if the scene wants something the brief doesn't name, say so. But an element the brief didn't ask for does not get built without Ben's authorization, and "the thing I needed it for required it" is not authorization. Unattended, build only what the brief names and write the proposal into `NIGHTLY-LOG.md` for morning review; if the brief genuinely cannot be built without the addition, that's `blocked`, not license. Attended, ask and wait for an answer. **Why this rule exists:** Firefly Summer's brief was a two-person wooden swing hanging in a meadow. Mid-build, a porch was invented to hang it from — post, beam, roof, chains, its own generated backdrop. Then the swing, the actual subject, was cut for being unreadable while the invented scaffolding stayed. The delivered scene was a support structure holding an empty chain where its subject should have been, and it cost a full asset-generation cycle plus two composition passes to discover. A structure invented to serve the subject will quietly become the subject.
