#!/usr/bin/env node
// Isolated A/B render of makeOccluder at fill=0.35 (min) vs fill=1.00 (max),
// same size/hue/position otherwise — controls out per-station noise (box
// size, star density) that made the real-station comparison inconclusive.
// Verifies task-2 acceptance: extent moves with fill, extent at min fill
// still clears a readable floor.
...
const targetUrl = `http://127.0.0.1:${port}/concepts/tools/occluder-isolated.html`;
const browser = await chromium.launch({ args: ['--no-sandbox'] });
const page = await browser.newPage({ viewport: { width: 900, height: 460 } });
await page.goto(targetUrl, { waitUntil: 'load' });
await page.waitForFunction(() => window.__ready === true);
await page.screenshot({ path: `${HERE}/occluder-isolated.png` });
...
// against a pure-black backdrop, threshold = any pixel brighter than near-black (k=8, matches
// ring-verify's spirit but against a solid-black surround the median is exactly 0)
const K = 8;
for (const [label, b] of [['fill=0.35 (min)', boxes.lo], ['fill=1.00 (max)', boxes.hi]]) {
  ...
  // surround is solid black here (isolated harness, no starfield) -> median=0,
  // so signal reduces to the box's own p95 luma, same formula ring-verify.mjs
  // uses (signal = p95(box) - median(surround)).
  const signal = p95Of(h) - 0;
